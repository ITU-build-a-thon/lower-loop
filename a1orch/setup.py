...
setup(
    ...
    url="",
    install_requires=["requests", "Flask","ricxappframe>=1.1.1,<2.0.0", "joblib>=0.3.2", "connexion[swagger-ui]","statsmodels>=0.11.1",  "schedule>=0.0.0", "mdclogpy<=1.1.1"],
    entry_points={"console_scripts": ["run-a1orch.py=a1orch.main:start"]},  # adds a magical entrypoint for Docker
    package_data={"a1orch": ["openapi.yaml"]},
)

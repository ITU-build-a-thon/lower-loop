...
class A1PolicyInterface:
    ...
    def send_orch_req(self, data: dict):
        logger.debug("Received data from A1: {}".format(data))
        self.pipeline = data
        src = self.pipeline['srcName'] 
        model = self.pipeline['modelName']
        sink = self.pipeline['sinkName']
        DATA = {
        'src':src,
        'model':model,
        'sink':sink,
